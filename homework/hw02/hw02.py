...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(i, result, dir):
        if i == n:
            return result
        
        if i % 8 == 0 or num_eights(i):
            return helper(i + 1, result - dir, -dir)
        else:
            return helper(i + 1, result + dir, dir)   
...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(MIN, MAX, diff_count, n):
        if n == 0:
            return MAX - MIN - diff_count + 1
        
        MIN = min(MIN, n % 10)
        MAX = max(MAX, n % 10)
        if n % 10 != n // 10 % 10:
            diff_count += 1
        return helper(MIN, MAX, diff_count, n // 10)
    
    return helper(9, 1, 0, n)
...
def count_coins(total):
    ...
    def helper(total, smallest_coin):
        if total == 0:
            ...
            return 0
        elif smallest_coin == 25:
            return helper(total - smallest_coin, smallest_coin)
        else:
